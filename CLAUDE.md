...
| `pago.ts` | `PagoSchema` / `IPago` — registro de pago genérico (sin pasarela). Discriminante `TipoPagoSchema` (hoy solo `Expensa`; se extiende al sumar multas) + ref polimórfica (`idExpensaUF`, futuro `idMulta`). `MedioPagoSchema` (Efectivo/Transferencia/Débito/Cheque/Otro). Soporta pagos parciales; acceso-api recalcula `montoPagado`/`estadoPago` del cargo al crear/borrar. Cuando llegue multas, el complejo configura si se pagan con la expensa (ítem) o aparte (`tipo='Multa'`) — la misma entidad sirve para ambos |
