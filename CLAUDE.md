...
| `paquete-firmware.ts` | `PaqueteFirmwareSchema` / `IPaqueteFirmware`, `VarianteFirmwareSchema`. Archivo de firmware subido por un admin, aplicable a los dispositivos de su modelo. `modelo` es **gate duro** del pre-flight (el binario equivocado es la causa dominante de brick); `version` es el criterio de éxito post-actualización. `archivo` = objectName del bucket **privado** (carpeta `firmware`); `sha256` lo calcula acceso-api y el edge lo revalida antes de mandarle nada al terminal. Cloud-only: el edge recibe una URL firmada dentro del comando, no el catálogo. |
