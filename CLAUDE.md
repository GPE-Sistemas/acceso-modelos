...
**Hardware** — `accesos`, `dispositivos`, `credenciales`, `dispositivos acceso`. Firmware: `Gestionar paquetes de firmware` (subir/retirar los binarios) y `Actualizar firmware dispositivo` (dispararla sobre UN equipo). Separadas a propósito: quien consigue y sube el binario no es necesariamente quien decide actualizar un terminal, y la actualización es irreversible (el fabricante bloquea el downgrade).
