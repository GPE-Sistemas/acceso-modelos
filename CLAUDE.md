...
|---|---|
| `acceso.ts` | `AccesoSchema` / `IAcceso`, `CreateAccesoSchema`, `UpdateAccesoSchema`, `TipoAccesoSchema`. **Estado runtime del actuador (D53)**: `idDispositivoAccesoActuador` (designa QUIÉN acciona el acceso — sin árbitro explícito, terminal + controlador en el mismo acceso es un empate), `estadoFisico` + **`confianzaEstado`** (`Reportado`/`Inferido`/`Desconocido`, obligatorio junto al estado: sin realimentación cableada el sistema sabe qué ordenó, no dónde quedó el fierro) + `fuenteEstado`/`fechaEstado`, y `estadoOperativo` (`Normal`/`Liberado`/`Bloqueado`) con `vigenteHasta`/`motivoEstadoOperativo`/`idPermisoEstadoOperativo`. El estado físico lo escribe el edge; el modo operativo es deseado gobernado por el sistema |
...
| `deteccion.ts` | `DeteccionSchema` / `IDeteccion`, `TipoDeteccionSchema` (persona/vehiculo/patente/rostro/acceso-terminal), `EstadoCorrelacionDeteccionSchema` (módulo IA-video M3). Nació como señal cruda de inferencia de video; **D56 la extiende a bitácora cruda de TODO dispositivo**: toda señal de un terminal de acceso se persiste acá — el intento validado, el no reconocido, y también puerta/relé/comando/alarma y lo que no sepamos interpretar. Bloque terminal: **`CategoriaDeteccionTerminalSchema`** (`Intento de Acceso`/`Puerta`/`Relé`/`Comando Remoto`/`Alarma`/`Sistema`/`No Mapeado` — `tipo` responde *de qué sensor viene*, `categoria` *qué clase de hecho es*; no va como valores de `TipoDeteccionSchema` porque ese enum lo consume `IIngresoEgreso.tipoDeteccion[]`), **`ResultadoDeteccionSchema`** (`Validada`/`No Reconocida`, solo para intentos), `identificador` (el crudo presentado: employeeNo/tarjeta/PIN — único dato del intento no reconocido), `modalidadAutenticacion` (espejo de `currentVerifyMode`; **ausente en terminal facial** hasta que se releven los modos con cara, pendiente de `credencial.ts`) y **`EventoOrigenDispositivoSchema`** (`major`/`minor` crudos, **siempre**, esté mapeado o no — es lo que vuelve accionable un `No Mapeado`) y **`IdentificacionDeteccionSchema`** (`idPermiso`/`nombre`/`unidadFuncional`, **snapshot** al momento de la detección: el `identificador` crudo no es legible porque en facial el `employeeNo` es el ObjectId del permiso, y va snapshot y no populate porque el Hub edge sirve plano y una detección es registro histórico — mismo criterio que `visitantesSnapshot`). `estadoCorrelacion` queda **AUSENTE** en las de terminal a propósito: el correlador filtra por `= 'Pendiente'`, así que el campo ausente las excluye por construcción y no duplica el `IIngresoEgreso` que ya crea el camino del terminal. Se persiste con `expireAt`/TTL. La configuración del `IDispositivoAcceso` decide si se materializa el `IIngresoEgreso`, **nunca** si se persiste la detección. |
| `dispositivo.ts` | `DispositivoSchema` / `IDispositivo`, `FormFactorDispositivoSchema` (+ `Cámara IP`/`NVR`/`XVR` de M1, + `Controlador de acceso`/`Controlador de I/O`/`Barrera` de D53), `ConfigDispositivoSchema` (+ `protocolo`/`rtspUriPlantilla`/`idPerfilCamara`/`canales`), `CapacidadesDispositivoSchema` (+ `deteccion`, + **`actuacion`**: `CapacidadesActuacionSchema` / `ComandoActuacionSchema` de D53 — salidas comandables, canales independientes, entradas de realimentación y comandos `pulso`/`mantenerAbierto`/`mantenerCerrado`/`detener`; se releva por sondeo del recurso, no por los flags `isSupport*`, y deprecia el booleano `aperturaComando`), `CapacidadesDeteccionSchema`, `ProtocoloDispositivoSchema`, `FuenteInferenciaSchema`, `CanalDispositivoSchema`. `capacidades.deteccion.identificacionRostro` gatea aprobado automático (decisión E). **`InventarioDispositivoSchema`** (owner edge, se pisa entero en cada reporte: `firmware`/`red`/`hora`/`ingesta`/`nube`/**`actuacion`** — la capacidad de actuación MEDIDA en el equipo (D53) vive acá y no en `capacidades.actuacion` porque `capacidades` también lo edita el operador y un reporte parcial del edge le pisaría el objeto entero; además así un equipo ya adoptado la consigue en el siguiente tick, sin re-adoptarlo. La capacidad EFECTIVA que valida acceso-api es lo declarado con fallback a lo medido, resuelto en la api y no persistido) y **`FirmwareDispositivoSchema`** (owner cloud, veredicto derivado del catálogo — `soporte`/`bloqueaEnrolamiento`). El edge lee el veredicto, no lo calcula. |
| `config-operacion-dispositivo.ts` | `ConfigOperacionDispositivoSchema` / `IConfigOperacionDispositivo` (D51) — configuración de OPERACIÓN del device (voz, textos en pantalla, apagado de pantalla, privacidad del cartel, imágenes por evento), **sparse a propósito**: campo ausente = no gestionado (el reconciliador no lo toca). Nombres propios, agnósticos de fabricante — el mapa a URIs ISAPI vive en el adapter del edge. Rangos declarados acá porque el `PUT` de ISAPI es todo-o-nada (un campo fuera de rango tumba el request completo). Además `ModoConfigDispositivoSchema` (**`Perfil` \| `Manual` \| `Sin gestionar`, excluyentes**: en `Perfil` el deseado sale entero del perfil y `configDeseada` va vacío; en `Manual` es al revés — `acceso-api` hace cumplir el invariante. Antes las dos capas se sumaban con el override pisando campo a campo, y un equipo podía dejar de seguir a su perfil sin decisión de nadie y sin síntoma: pasó en producción. Cloud-only, el edge recibe el deseado ya resuelto), `EstadoConfigDispositivoSchema` (Coincide/No coincide/Pendiente/No aplicable — `No aplicable` se muestra "Sin gestionar" en la UI, el valor del enum no se renombra porque vive en constantes Go del edge y en datos de producción), `DiffConfigDispositivoSchema` (`campo` dot-path + `esperado`/`real`/`recurso`) y `ConfiguracionReportadaDispositivoSchema` (bloque que reporta el edge: `real`, `soportados[]`/`noSoportados[]`, `diffs[]`, back-off). Alcance = F1; los grupos de F2 (umbrales faciales, puerta, planes horarios) se suman al implementarlos |
| `perfil-dispositivo.ts` | `PerfilDispositivoSchema` / `IPerfilDispositivo` (D51) — configuración de operación con nombre, reutilizable. Del cliente (opcionalmente acotada a un complejo), **no** catálogo curado por GPE como `IPerfilCamara`. `marca`/`modelo` acotan elegibilidad; `config` son los knobs que gobierna — **enteros**, sin mezclarse con configuración propia del equipo (modos excluyentes), que es lo que hace que cambiar el perfil mueva a todos sus terminales. **Cloud-only**: el edge recibe el deseado ya resuelto en el dispositivo, no el catálogo. NO declara `formFactor` (evita el ciclo de imports con `dispositivo.ts`, que importa este schema para el populate) |
| `dispositivo-acceso.ts` | `DispositivoAccesoSchema` / `IDispositivoAcceso`, `ComportamientoCredencialValidaSchema` (**`Aprobación Automática` | `Aprobación Manual`** — v3.0.0 renombró `Apertura Automática`: el campo decide sólo el estado del `IIngresoEgreso`, nunca si se acciona el fierro; eso sale de la actuación), `ComportamientoCredencialInvalidaSchema`, + (M4) `RolEnEventoSchema`, `ComportamientoDeteccionSchema`, `DisparoDeteccionSchema` (cadenas de detección). **`ActuadorDispositivoAccesoSchema` (D53, Capa 2)**: `RolActuacionSchema` (rol de ACTUACIÓN, distinto de `rolEnEvento` que es del plano evento), `ModoActuacionSchema` (`PulsoUnico`/`AbrirCerrar`/`AbrirCerrarStop`/`Mantenida`), mapeo a salidas físicas (`salidaAbrir`/`salidaCerrar`/`salidaDetener` — es lo que permite que un controlador de I/O maneje N barreras: N filas, cada una con su juego de salidas), tiempos (`pulsoMs`/`tiempoRecorridoSeg`/`autoCierreSeg`) y `FeedbackActuacionSchema` (`Ninguna`/`RelayDelActuador`/`SensorFisico` — qué está CABLEADO, no qué soporta el hardware). `modo` ⊆ `capacidades.actuacion.comandos`, validado cloud-side. **`ControlAperturaSchema` (`Dispositivo` | `Sistema`)**: quién abre en el flujo automático — `Dispositivo` = el equipo concede por su padrón y acciona su salida solo (lo que hace el HIK, medido), y el sistema no manda un pulso redundante; `Sistema` = la apertura la ordena el sistema (terminal que no acciona, actuador que es otro equipo, o validación que el device no puede hacer). Ausente ⇒ `Dispositivo`, para no cambiarle el comportamiento a una instalación andando. Ortogonal a la operatoria manual del guardia, que funciona siempre. **`aperturaConComando` ELIMINADO** en v3.0.0: la capacidad de comandar sale de `actuador` + capacidad efectiva |
| `actuacion.ts` | Contrato de **operación del actuador** (D53): `AccionActuacionSchema` (`abrir`/`cerrar`/`detener` — lo que ordena el operador, distinto del `ComandoActuacionSchema` que entiende el hardware), `OperarAccesoSchema` / `OperarAccesoResponseSchema` (body y resultado de `POST /accesos/:id/operar`; `resultado:'ok'` = el actuador aceptó la orden, **nunca** que el fierro se movió) y `ActuacionAccesoSchema` (respuesta de `GET /accesos/:id/actuacion`: todo lo que la UI necesita para pintar el control sin recalcular reglas). Constantes runtime **`ACCIONES_POR_MODO_ACTUACION`**, `COMANDOS_REQUERIDOS_POR_MODO` y `SALIDAS_REQUERIDAS_POR_MODO` — fuente única de las reglas: acceso-api rechaza la acción inválida y acceso-web arma los botones con lo mismo, la UI no reimplementa nada. Archivo propio (no `acceso.ts`) porque cruza las 3 capas y `dispositivo-acceso.ts` ya importa `AccesoSchema` para el populate — importarlo de vuelta armaría un ciclo |
...
| `estadia.ts` | `EstadiaCalculadaSchema` / `IEstadiaCalculada`, `SujetoEstadiaSchema`, `PresenciaQuerySchema`, `EstadoPresenciaSchema`, constante `VENTANA_PRESENCIA_HORAS_DEFAULT` (D58, doc 45). **No es una entidad**: la estadía es el intervalo de permanencia **por sujeto** (tres visitantes que entran juntos y salen en dos tandas = 1 ingreso, 2 egresos, **3 estadías**; el movimiento es multi-sujeto por diseño, así que el par no se expresa con un puntero movimiento → movimiento). Se calcula plegando el stream de `IIngresoEgreso` y este archivo declara la forma de la respuesta de `GET /presencia` para que cloud (Mongo) y edge (Postgres) devuelvan lo mismo — el fold vive en un solo lugar por motor, antes estaba escrito dos veces en el front con reglas distintas. `idEventoVisita`/`idPermiso` **referencian** la autorización sin reemplazarla (un evento recurrente son N estadías; un propietario no tiene evento). Los movimientos van populados como `z.any()` (criterio de `vinculo-evento-ingreso.ts`) y el contenido **no** se denormaliza: la estadía es un índice. No hay estado `Vencida` porque sin materializar el fold no tiene dónde escribir un cierre por política. La entidad `IEstadia` persistida es el objetivo de la fase 3, y se crea recién cuando haya un consumidor que necesite el ancla (tracking D55, ANPR D56, reportes de permanencia). **`UltimoMovimientoPermisoSchema` / `IUltimoMovimientoPermiso`** es otra pregunta, no el fold: dónde quedó parado UN permiso (`dentro` + el movimiento que lo dejó así), **sin ventana** — lo consume la ficha de la unidad funcional, donde el último movimiento del integrante puede ser de hace un mes y "no figura" no puede leerse como "está afuera". El sujeto se mira desde los dos lados del movimiento (`idPermiso` y `idsPermisosAcompanantes`) |
| `paquete-firmware.ts` | `PaqueteFirmwareSchema` / `IPaqueteFirmware`, `VarianteFirmwareSchema`. Archivo de firmware subido por un admin, aplicable a los dispositivos de su modelo. `modelo` es **gate duro** del pre-flight (el binario equivocado es la causa dominante de brick); `version` es el criterio de éxito post-actualización. `archivo` = objectName del bucket **privado** (carpeta `firmware`); `sha256` lo calcula acceso-api y el edge lo revalida antes de mandarle nada al terminal. **`formatoArchivo`** (`dav` | `zip`) permite subir el ZIP tal como lo entrega el fabricante: pesa la mitad que el binario y evita descomprimir a mano (la herramienta estándar de macOS no maneja bien el anidado). Lo detecta acceso-api por los primeros bytes del objeto, no por la extensión; el edge extrae el `.dav` recursivamente después de verificar el hash de lo descargado. Cloud-only: el edge recibe una URL firmada dentro del comando, no el catálogo. |
...
| `solicitud-permiso.ts` | `SolicitudPermisoSchema` / `ISolicitudPermiso`, `EstadoSolicitudPermisoSchema` (Pendiente/Aprobada/Rechazada/Cancelada), `SolicitanteSolicitudPermisoSchema` (snapshot de datos del solicitante), `ResolucionSolicitudPermisoSchema`, `MetadataUbicacionSolicitudPermisoSchema`. Solicitud de acceso a una UF hecha por un usuario **sin permisos** desde dentro del polígono del complejo: `textoUnidadFuncional` es texto libre (el match a una UF real lo hace un admin al aprobar) y `ubicacion` (Point) es obligatoria — acceso-api resuelve `idComplejo`/`idCliente` por `$geoIntersects`, el cliente nunca los manda. Una sola Pendiente por (usuario, complejo). Cloud-only |
...
| `rol.ts` | `RolSchema` / `IRol` — discriminated union por `alcance`. `AccionesRolSchema` enumera todas las acciones del catálogo. D53 suma `Hardware - Operar acceso` (abrir/cerrar/detener, rutina del guardia) y `Hardware - Cambiar modo operativo del acceso` (liberar/bloquear, decisión de supervisión que persiste) — separadas de `Hardware - Apertura remota dispositivo`, que es comando directo al device para mantenimiento. **Toda acción nueva hay que agregarla a `acciones-grupos.ts` de `acceso-web` en el mismo ciclo, o queda en 403 permanente** |
...
| `canal-notificacion.ts` | `CanalNotificacionSchema` (`emergencia`, `operacion`, `urgente_vecino`, `vecino`), `CANALES_NOTIFICACION` (id del canal Android, `interruption-level` iOS, sonido), `TipoPushSchema` (todos los `data.type` que emite acceso-api), `TIPOS_PUSH_URGENTES`, `canalNotificacion(audiencia, tipo, data)`. **La audiencia la decide el nivel del permiso destinatario** (Complejo = `personal`, UF o sin permiso = `vecino`), no el tipo — `ticket_mensaje` va a los dos. La urgencia la decide el tipo. Cambiar sonido o importancia de un canal exige `idCanalAndroid` nuevo (Android no deja cambiarlos una vez creado). `emergencia` usa `time-sensitive` hasta tener el entitlement de critical alerts de Apple. `CATEGORIAS_NOTIFICACION_OBLIGATORIAS` suma `seguridad_evento` y `ticket_emergencia_recibido` (3.18.0) |
...
| `dashboard.ts` | `DashboardComplejoSchema` / `IDashboardComplejo`, `DashboardUFSchema` / `IDashboardUF`, `DashboardClienteSchema` / `IDashboardCliente`, `DashboardProveedorSchema` / `IDashboardProveedor`. Dos ejes ortogonales sobre la misma UF del mapa: `CoberturaUFMapaSchema` (vinculación + facial) y `DashboardComplejoPresenciaSchema` + `residentesUF`/`residentesAdentro` (quién está adentro, plegando estadías D58 — con gate `soportaPresencia` porque sin control de egreso la estadía no cierra nunca). `movimientos.personasDentroEstimado` queda deprecado: cortaba a medianoche |
...
| `protocolo.ts` | `ProtocoloSchema` / `IProtocolo` — guía operativa configurable por complejo (catálogo N/complejo, NO singleton). **Combina** (todos opcionales) `descripcion` (texto plano), `archivo`+`archivoMeta` (PDF privado GCS `protocolos/<idComplejo>`, 1/protocolo) y `pasos[]` (`PasoProtocoloSchema` subdoc `_id:true`: `orden`/`titulo`/`descripcion?`/`obligatorio?`/`requiereComentario?`/`requiereFoto?`, max `MAX_PASOS_PROTOCOLO=50`, patrón `encuesta.preguntas`). `auditable` gatea si la ejecución se persiste (`IEjecucionProtocolo`) o es solo guía visual. `bloqueaCierre` (default off): obligatorios incompletos bloquean cierre del evento. **Asociación polimórfica** `aplicaA: AplicaAProtocoloSchema` (discriminated `tipo`: `Generico` \| `BotonTicket{idBoton}` \| `CategoriaTicket{categoria}` \| `IngresoEgreso{categoria}`), un solo target. `activo` filtra en `/protocolos/aplicables`. Cloud + mobile |
| `ejecucion-protocolo.ts` | `EjecucionProtocoloSchema` / `IEjecucionProtocolo` — traza de ejecución (solo si `protocolo.auditable`). `protocoloSnapshot` (`ProtocoloSnapshotSchema` = `pick` de nombre/descripcion/pasos/archivo/bloqueaCierre) congelado al iniciar → resuelve versionado sin versionar (patrón `botonSnapshot`). `origen: OrigenEjecucionProtocoloSchema` (discriminated `tipo`: `Ticket{idTicket}` \| `IngresoEgreso{idIngresoEgreso}` \| `Generico`). `EstadoEjecucionProtocoloSchema` (EnCurso/Completada/Abandonada). `pasos: PasoEjecutadoSchema[]` (`idPaso`+`titulo` snapshot, `completado`/`fechaCompletado`/`idPermiso`/`comentario`/`foto?` objectName). `idPermiso` = operador |
...
`AccionesRolSchema` (`src/interfaces/rol.ts`) es la fuente de verdad. Módulos: `Administración`, `Hardware`, `Visitas`, `Vehículos`, `Mascotas`, `Movimientos`, `Publicaciones`, `Tickets`, `Turnos`, `Encuestas`, `Expensas`, `Infracciones`, `Egresos`, `Obras`, `Protocolos`.

**Protocolos** (guías operativas configurables, cloud + mobile) — Configuración (Complejo): `Ver/Crear/Editar/Eliminar protocolos`. Ejecución (gate propio multi-dominio, independiente de atender tickets / registrar ingreso-egreso): `Ejecutar protocolos`, `Ver ejecuciones`.
...
**Hardware** — `accesos`, `dispositivos`, `credenciales`, `dispositivos acceso`. Configuración de operación del device (D51): `Configurar dispositivos` (declarar el deseado y forzar reconciliación — separada de `Editar dispositivos` porque escribe en el hardware, no es master-data) y `Ver/Crear/Editar/Eliminar perfiles dispositivo` (catálogo de perfiles del cliente). Firmware: `Gestionar paquetes de firmware` (subir/retirar los binarios) y `Actualizar firmware dispositivo` (dispararla sobre UN equipo). Separadas a propósito: quien consigue y sube el binario no es necesariamente quien decide actualizar un terminal, y la actualización es irreversible (el fabricante bloquea el downgrade).
