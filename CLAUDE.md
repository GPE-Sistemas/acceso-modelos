...
| `visitante.ts` | `VisitanteSchema` / `IVisitante`. Campos `activo?: boolean` + `idPermisoCreador?: string` (soft-archive — índices únicos parciales sobre teléfono/DNI filtran por `activo: true`). Omitidos de Create/Update — los inyecta `acceso-api`. Campos `validadoPorInvitado?: boolean` + `fechaUltimaValidacionInvitado?: string` (el invitado completó sus datos vía link público — ver `invitacion-visita.ts`); también omitidos de Create/Update, los setea el endpoint público |
...
`AccionesRolSchema` (`src/interfaces/rol.ts`) es la fuente de verdad. Módulos: `Administración`, `Hardware`, `Visitas`, `Vehículos`, `Movimientos`, `Publicaciones`, `Tickets`, `Turnos`, `Encuestas`, `Expensas`, `Infracciones`, `Egresos`.
